...
def get_line_string(start_x: int, start_y: int, end_x: int, end_y: int):
    return f'<line x1="{start_x}" y1="{start_y}" x2="{end_x}" y2="{end_y}" stroke="black" />'
...
def generate_svg_from_tree(trunk: Trunk) -> None:
    ...
    svg_content = []
    svg_content.append(f'<svg viewBox="{viewport[0]} {viewport[1]} {viewport[2]} {viewport[3]}" xmlns="http://www.w3.org/2000/svg">')
    for line in lines:
